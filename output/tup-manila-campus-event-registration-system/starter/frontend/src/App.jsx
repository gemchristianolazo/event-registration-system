...
const currentUser = { name: 'Mika Santos', email: 'mika.santos@tup.edu.ph', id: '2023-00142' }
...
function ConfirmationPage({ events }) {
  ...
  return <main className="confirmation-page"><div className="success-mark"><Check {...iconProps} /></div><div className="eyebrow">REGISTRATION CONFIRMED</div><h1>You're going to<br /><em>{event.title}.</em></h1><p className="confirmation-lead">Your spot is saved, Mika. Keep this pass handy for check-in.</p><div className="ticket"><div className="ticket-info"><span className="category-tag">{event.category}</span><h2>{event.title}</h2><p>{event.date} / {event.time}</p><p>{event.venue}</p><div className="ticket-id">TICKET ID <b>TE-2026-00142</b></div></div><div className="qr-wrap"><QRCodeSVG value={`tup-events:${event.id}:TE-2026-00142`} size={156} bgColor="#fffdf8" fgColor="#14251f" level="M" /><small>Scan at check-in</small></div></div><div className="confirmation-actions"><Link className="button button-primary" to="/my-events">View my events <ArrowRight {...iconProps} /></Link><Link className="text-link" to="/">Browse more events</Link></div></main>
}
